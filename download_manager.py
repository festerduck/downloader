...
"""
import argparse, asyncio, contextlib, hashlib, json, math, random, re, ssl, time
...
class Manager:
    ...
    async def worker_task(self, session, url, host):
        t0 = time.time()
        try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, headers=hdrs, cookies=cookies, connector=connector) as session:
            ...
            while self.urls or self.inflight:
                ...
                while self.urls and len(self.inflight) < self.cur:
                    ...
                    url, host = picked
                    self.host_busy[host] += 1
                    t = asyncio.create_task(self.worker_task(session, url, host))
                    self.inflight.add(t)
                    launched = True
